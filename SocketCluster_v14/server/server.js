...
const SOCKETCLUSTER_LOG_LEVEL = process.env.SOCKETCLUSTER_LOG_LEVEL || 4;
...
(async () => {
	for await (let {socket} of agServer.listener('connection')) {
		...
		(async () => {
			for await (let data of socket.receiver('QRSocket')) {
				console.log("QRSocket", data);
				console.log("data.channels", data.channels);
				socket.exchange.transmitPublish(data.channels.private, {'event': {name: 'QRSocket'}, data: data});
			}
		})();
	}
})();
